...
def hft_system(symbol, account_balance, risk_tolerance, stop_loss_level, take_profit_level):
    ...
    # Calculate RSI and MACD for the whole dataset
    data['RSI'] = talib.RSI(data['close'], timeperiod=14)
    macd, macd_signal, macd_hist = talib.MACD(data['close'], fastperiod=12, slowperiod=26, signalperiod=9)
    data['MACD_Hist'] = macd_hist

    # Iterate over the data starting from the 34th row
    for index in range(33, len(data)):
        ...
        # Update the conditions to include RSI and MACD
        if not long_position and price > ma and rsi < 30 and macd_hist_current > macd_hist.iloc[index - 1]:
            long_position = True
            entry_price = price
